...
I used the carseats dataset for both questions.
...
#grow tree
fit <- rpart(Sales~., subset = train, method="anova", data=Carseats)
...
A regression tree was plotted for the carseats dataset with sales as the response variable and the other variables as input variables. The relevant variables of the tree are ShelveLoc, Price, CompPrice, Advertising and Age. 
...
As seen in the plot above, for both the testing and training sets, the error reduces as the number of mtry increases. However, the training error seems to be less than the test error the whole time. 
...
boost.err1 = with(Carseats[-train,], apply( (predmat- Sales)^2, 2, mean) )
plot(n.trees, boost.err1, pch = 23, ylab = "Mean Squared Error", xlab = "# Trees", main = "Boosting Test Error")
abline(h = min(test.err), col = "red")
```

As seen in the plot above, the mean squared error reduces with the number of trees increases. The error reaches a minimum at about 1000 trees. Then it increases and comes to a level at last.
